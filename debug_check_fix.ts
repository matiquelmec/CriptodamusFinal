...
import { IndicatorCalculator } from './src/services/engine/pipeline/IndicatorCalculator';
...
try {
    console.log("Running IndicatorCalculator Test (Root)...");
    ...
    if (result.pivots) {
        ...
        console.error("❌ Failure: 'pivots' property is MISSING. Found: " + JSON.stringify(Object.keys(result).filter(k => k.includes('pivot'))));
        process.exit(1);
    }
    ...
    if (result.bollinger) {
        ...
        console.error("❌ Failure: 'bollinger' property is MISSING. Found: " + JSON.stringify(Object.keys(result).filter(k => k.includes('boll'))));
        process.exit(1);
    }
