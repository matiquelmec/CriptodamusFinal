...
export class EconomicService {
    ...
    private static CSV_URL_PRIMARY = 'https://nfs.faireconomy.media/ff_calendar_thisweek.csv';
    private static CSV_URL_BACKUP = 'https://nfs.faireconomy.media/ff_calendar_thisweek.xml'; // Fallback (or other source)
    // REAL BACKUP: Often we can use a raw GitHub gist or valid CSV mirror if primary fails
    ...
    private static sourceStatus: 'LIVE' | 'BACKUP' | 'CACHE' | 'EMPTY' = 'EMPTY';
    ...
    private static async checkNuclearStatusForTime(dateToCheck: Date): Promise<ShieldStatus> {
        try {
            ...
            const isDegraded = this.sourceStatus === 'EMPTY'; // NEW: Degraded if totally empty
            const todayStr = dateToCheck.toISOString().split('T')[0]; // YYYY-MM-DD

            // 0. FAIL-SAFE: If DEGRADED (Empty), do not block, but warn.
            if (isDegraded && events.length === 0) {
                return {
                    isActive: false, // DO NOT BLOCK
                    isImminent: false,
                    isCached: false,
                    reason: 'NEWS_BLINDNESS_DEGRADED_MODE: System operating with technicals only. Risk penalized.'
                };
            }
            ...
            return {
                isActive: true, // Only Active if Risk is Real
                isImminent,
                isCached,
                reason,
            ...
            console.warn("[EconomicService] ⚠️ Critical Shield Failure (Degraded Fallback):", e.message);
            // FAIL-SAFE 2: If Logic Crashes, Default to Degraded, NOT Blocked
            return {
                isActive: false,
                isImminent: false,
                reason: 'SYSTEM_ERROR_DEGRADED_MODE: Economic Guard crashed. Defaulting to technicals.'
            };
        }
    }
    ...
    private static async fetchEvents(): Promise<EconomicEvent[]> {
        ...
        // 4. DEGRADED MODE (Return Empty but marked as EMPTY status)
