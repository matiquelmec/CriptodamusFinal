...
class SmartFetch {
     ...
     * Smart GET request
     */
    static async get(url, config = {}, retries = 3) {
        // 1. Deduplication
        ...
        const requestPromise = this.executeRequest(url, config, retries);
        // Store the promise
    ...
    static async executeRequest(url, config, retriesLeft) {
        ...
        if ((isBinance || isForex) && process.env.BIFROST_URL && !isBifrost) {
            ...
            return this.executeRequest(bifrostUrl, config, retriesLeft);
        }
        // --- STEALTH HEADERS (ENGAGE) ---
        // Modern browsers send Client Hints to pass Cloudflare Challenges
        const stealthHeaders = {
            'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36',
            'Accept': ((_a = config.headers) === null || _a === void 0 ? void 0 : _a.Accept) || 'application/json, text/plain, */*',
        ...
        try {
            ...
            const contentType = (_b = response.headers['content-type']) === null || _b === void 0 ? void 0 : _b.toString().toLowerCase();
            const body = ((_c = response.data) === null || _c === void 0 ? void 0 : _c.toString()) || '';
            ...
            if (isCloudflare || (contentType && (contentType.includes('text/html') || contentType.includes('application/xhtml+xml')))) {
                ...
                if (process.env.BIFROST_URL && !isBifrost && retriesLeft > 0) {
                    ...
                    const bifrostUrl = `${process.env.BIFROST_URL}/api?target=${encodeURIComponent(url)}`;
                    return this.executeRequest(bifrostUrl, config, retriesLeft - 1);
                }
            ...
            if (((_d = error.response) === null || _d === void 0 ? void 0 : _d.status) === 403) {
            ...
            if (shouldRetry) {
                ...
                console.warn(`[SmartFetch] Fetch failed for ${domain} (${error.code || ((_f = error.response) === null || _f === void 0 ? void 0 : _f.status)}). Retrying in ${delay}ms...`);
                await new Promise(r => setTimeout(r, delay));
                return this.executeRequest(url, config, retriesLeft - 1);
            }
            throw error;
        }
    }
    static async enforceRateLimit(domain) {
...
SmartFetch.pendingRequests = new Map();
SmartFetch.lastRequestTime = new Map();
SmartFetch.MIN_DELAY_PER_DOMAIN = 2000; // 2 seconds safety buffer
// IPv4 Agent to avoid ENETUNREACH in some environments
SmartFetch.ipv4Agent = new https_1.default.Agent({ family: 4 });
