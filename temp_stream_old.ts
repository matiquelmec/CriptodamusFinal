...
class BinanceStreamService {
    ...
    constructor() {
        ...
        if (SUPABASE_URL && SUPABASE_KEY) {
            ...
            console.warn('ÔÜá´©Å [BinanceStream] No Supabase credentials. Real Data Accumulator disabled.');
        }
    }
    ...
    private async flushLiquidations() {
        ...
        try {
            ...
            if (error) {
                console.error('ÔØî [BinanceStream] Error flushing liquidations:', error.message);
            } else {
                console.log(`­ƒÆ¥ [BinanceStream] Successfully flushed ${batchSize} liquidations to Supabase.`);
            }
        } catch (e) {
            console.error('ÔØî [BinanceStream] Flush exception:', e);
        }
    }
    ...
    public start(): void {
        ...
        this.ws.on('open', () => {
            console.log('Ô£à [BinanceStream] Connected to Binance Futures WebSocket.');
            this.isAlive = true;
            this.heartbeat();

            // Re-subscribe to all (in case we only connected with a subset or none)
            if (this.streams.size > 0) {
                console.log(`­ƒôí [BinanceStream] Subscribing to ${this.streams.size} streams...`);
