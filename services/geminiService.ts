...
export const streamMarketAnalysis = async function* (
    ...
    if (isAnalysisRequest) {
        ...
        // Add SFP Alert
        if (isSFP) {
            const sfpText = sfpType === 'BEARISH_SWEEP' ? '🔴 SWEEP BAJISTA' : '🟢 SWEEP ALCISTA';
            response += `| **⚡ ALERTA SFP** | **${sfpText}** | Patrón de reversión detectado. |\n`;
        }

        // SQUEEZE DETECTION
        const isSqueeze = bollinger.bandwidth < 0.15;
        if (isSqueeze) {
            const squeezeDir = techData.trendStatus.emaAlignment === 'BULLISH' ? 'ALCISTA' : 'BAJISTA';
            response += `| 🔥 SQUEEZE | **COMPRESIÓN ALTA** (Ruptura ${squeezeDir} Probable) |\n`;
        }
