...
class SignalAuditService extends EventEmitter {
    ...
    public async start() {
        console.log("­ƒøí´©Å [SignalAudit] Auditor├¡a de Se├▒ales Iniciada (Elite Mode: Stages & Real PnL).");
    ...
    private async reloadActiveSignals() {
        ...
        if (!error && data) {
            ...
            console.log(`­ƒøí´©Å [SignalAudit] Seguimiento activo de ${data.length} se├▒ales (DCA Hydrated).`);
            ...
            try {
                ...
                if (currentPrice > 0) {
                    ...
                    if (canEnterNow) {
                        ...
                        console.log(`ÔÜí [SignalAudit] Market Entry: ${opp.symbol} @ $${activationPrice.toFixed(4)} (Slip: ${slippageAmount.toFixed(4)})`);
                    }
                }
                ...
                if (!error && data) {
                    ...
                    binanceStream.addStream(streamSymbol);
                    console.log(`Ô£à [SignalAudit] Registered: ${opp.symbol} ${opp.side} (PRO Strategy)`);
    ...
    private async processPriceTick(symbol: string, currentPrice: number) {
        ...
        if (!currentPrice || Number.isNaN(currentPrice) || currentPrice <= 0 || typeof currentPrice !== 'number') {
            systemAlerts.logAlert({
                symbol,
                severity: 'MEDIUM',  // Ô£à DOWNGRADED: HIGH ÔåÆ MEDIUM (tick validation is not critical)
            ...
            try {
                ...
                if (signal.status === 'PENDING' || signal.status === 'OPEN') {
                    ...
                    if (entryTouched) {
                        ...
                        console.log(`­ƒÜÇ [SignalAudit] Filled E1: ${signal.symbol} @ $${realEntry.toFixed(4)}`);
                    }
                }
                ...
                if (isActive) {
                    ...
                    if (currentStage === 0 && signal.dcaPlan) {
                        ...
                        if (e2 && !signal.e2_filled) {
                            ...
                            if (hitE2) {
                                ...
                                console.log(`­ƒôí [SignalAudit] DCA E2 Filled: ${signal.symbol} (New WAP: $${currentWAP.toFixed(4)})`);
                            }
                        }
                        ...
                        if (e3 && signal.e2_filled && !signal.e3_filled) {
                            ...
                            if (hitE3) {
                                ...
                                console.log(`­ƒôí [SignalAudit] DCA E3 Filled: ${signal.symbol} (Final WAP: $${currentWAP.toFixed(4)})`);
                            }
                        }
                    }
                    ...
                    if (currentStage >= 1) {
                        ...
                        if (tradeAgeHours < MIN_AGE_FOR_TRAILING) {
                            ...
                            // console.log(`ÔÅ│ [Time-Lock] ${signal.symbol}: Trade too young for trailing (${tradeAgeHours.toFixed(1)}h < ${MIN_AGE_FOR_TRAILING}h)`);
                        } else {
                            ...
                            const TRAILING_ACTIVATION = TradingConfig.exit_strategy.trailing_stop.min_profit_to_activate / 100; // 2.5% ÔåÆ 0.025
                            ...
                            if (peakProfitPct >= TRAILING_ACTIVATION) {
                                ...
                                const trailDistance = atr * TRAILING_DISTANCE_ATR; // e.g. 2 ├ù ATR
                                ...
                                // console.log(`­ƒÅâ [Trailing] ${signal.symbol}: Peak +${(peakProfitPct*100).toFixed(2)}% ÔåÆ Trail SL: $${trailLevel.toFixed(2)} (ATR ${atr.toFixed(2)})`);
                            }
                            ...
                            if (isLong) {
                                ...
                                if (improvement > 0.005) {
                                    ...
                                    // console.log(`­ƒôê [SL-Move] ${signal.symbol}: $${sl.toFixed(2)} ÔåÆ $${bestNewSL.toFixed(2)} (+${(improvement*100).toFixed(2)}%)`);
                                } else {
                                    effectiveSL = sl; // Keep current (noise filter)
                                    // console.log(`­ƒöç [Noise-Filter] ${signal.symbol}: SL unchanged (improvement ${(improvement*100).toFixed(2)}% < 0.5%)`);
                                }
                            } else {
                    ...
                    if (currentStage === 0 && signal.created_at) {
                        ...
                        if (tradeAgeHours >= TIME_DECAY_HOURS) {
                            ...
                            if (canBreakeven) {
                                console.log(`ÔÅ░ [Time-Decay] ${signal.symbol}: 12h limit reached. Forcing Breakeven Exit.`);
                                shouldClose = true;
                                ...
                                if (isLong ? effectiveSL < forcedBE : effectiveSL > forcedBE) {
                                    ...
                                    console.log(`ÔÅ░ [Time-Decay] ${signal.symbol}: Moving SL to Breakeven after 12h.`);
                                }
                            }
                        }
                    }
                    ...
                    if (effectiveSL !== sl) {
                        ...
                        if (Math.abs(effectiveSL - (signal.activation_price || signal.entry_price)) < (currentWAP * 0.01)) {
                            ...
                            // console.log(`­ƒøí´©Å [SignalAudit] ${signal.symbol}: Moving SL to Breakeven (${effectiveSL})`);
                        }
                    }
                    ...
                    if (Date.now() - lastSync > 30000) { // Sync every 30s
                        ...
                        if ((signal.status === 'PARTIAL_WIN' || signal.status === 'WIN') && signal.entry_price) {
                            ...
                            if (!isSecured) {
                                console.warn(`­ƒøí´©Å [Integrity-Fix] Upgrading Secured Trade ${signal.symbol} to Smart BE ($${smartBE}).`);
                    ...
                    if (slHit) {
                        ...
                        updates.final_price = currentPrice;
                        if (currentStage > 0) {
                            console.log(`­ƒøí´©Å [SignalAudit] Smart Breakeven Hit: ${signal.symbol} (Secured Profit)`);
                            ...
                            console.log(`Ôøö [SignalAudit] Technical Stop Loss Hit: ${signal.symbol} @ $${currentPrice}`);
                        }
                    } else {
                            ...
                            if (tp1Hit) {
                                ...
                                if (entryPrice) {
                                    ...
                                    if (betterSL !== signal.stop_loss) {
                                        ...
                                        console.log(`­ƒøí´©Å [TP1-Trigger] Forcing SL to Breakeven: ${betterSL}`);
                                    }
                                }

                                console.log(`­ƒÆ░ [SignalAudit] TP1 Hit: ${signal.symbol} (50% Secured | Smart Breakeven Active)`);
                        ...
                        // TP2 (30% out)
                            ...
                            if (tp2Hit) {
                                ...
                                console.log(`­ƒÆ░­ƒÆ░ [SignalAudit] TP2 Hit: ${signal.symbol}`);
                                telegramService.sendUpdateAlert('TP_HIT', {
                                    symbol: signal.symbol,
                                    stage: 2,
                                    price: tp2,
                        ...
                        // TP3 (30% out / Final Moon)
                            ...
                            if (tp3Hit) {
                                ...
                                console.log(`­ƒÜÇ­ƒÜÇ­ƒÜÇ [SignalAudit] TP3 Target Reached: ${signal.symbol}`);
                            }
                        }
                    }
                }
                ...
                // PROTOCOLO DE SEGURIDAD: Si falla el c├ílculo, NO PODEMOS CONFIAR en la se├▒al.
    ...
    private async syncUpdates(updates: any[]) {
        for (const upd of updates) {
            ...
            if (upd.status === 'WIN' && (upd.pnl_percent || 0) <= 0) {
                console.warn(`­ƒøí´©Å [SignalAudit] Integrity Guard: Converting WIN to LOSS for ${upd.id} due to non-positive PnL (${upd.pnl_percent?.toFixed(2)}%)`);
                upd.status = 'LOSS';
            }
            ...
            if (id) {
                ...
                // console.log(`­ƒÆ¥ [Sync] Trace: Updating ${id}, fields:`, Object.keys(data));
                ...
                if (dbError) {
                    console.error(`ÔØî [Sync] FAILED to persist update for ${id}:`, dbError.message);
                } else {
                    if (data.stop_loss) {
                        console.log(`Ô£à [Sync] DB WRITE SUCCESS: SL for ${id} set to ${data.stop_loss}`);
                    }
                    ...
                    if (upd.status === 'WIN' || upd.status === 'LOSS') {
                        ...
                        console.log(`­ƒÄ» [SignalAudit] CERRADA (${upd.status}): ${upd.id} | Net PnL: ${finalPnL.toFixed(2)}%`);
                        ...
                        this.updateMLOutcome(upd.id, finalPnL, upd.status).catch(e =>
                            console.error(`ÔÜá´©Å [ML-Audit] Failed to sync outcome:`, e)
                        );
                    }
                }
            }
    ...
    private async updateMLOutcome(signalId: string, pnl: number, status: string) {
        ...
        if (predictions && predictions.length > 0) {
            ...
            console.log(`­ƒºá [ML-Audit] Loop Closed: Prediction ${pred.id} marked as ${outcome === 1 ? 'ACCURATE' : 'INACCURATE'} (PnL: ${pnl.toFixed(2)}%)`);
        }
    }
    ...
    private async checkExpirations() {
        ...
        for (const signal of this.activeSignals) {
            ...
            if (signal.status === 'PENDING' || signal.status === 'OPEN') {
                ...
                if (ageHours > limit) {
                    console.log(`Ôîø [SignalAudit] Expiring stale signal: ${signal.symbol}`);
     ...
     * 3. Divergencia Regular ÔåÆ Reversi├│n detectada
     * 4. Momentum Exhaustion ÔåÆ RSI >70 sin movimiento
     * 5. Trailing Stop ÔåÆ Despu├®s de TP1 (ATR-based)
     * 6. Time Decay (SOFT) ÔåÆ Estrecha SL 6-12h
     * 7. Max Time (HARD) ÔåÆ Solo trades FLAT >12h
     */
                ...
                try {
                    ...
                    if (exit_strategy.reversal?.enabled) {
                        ...
                        if (isLong && detectRegularBearishDivergence(candles, rsiArray, exit_strategy.reversal.lookback_candles)) {
                            ...
                            console.log(`­ƒöä [AdvancedExit] ${signal.symbol}: Bearish Divergence (Reversal)`);
                        }
                        ...
                        if (!isLong && detectRegularBullishDivergence(candles, rsiArray, exit_strategy.reversal.lookback_candles)) {
                            ...
                            console.log(`­ƒöä [AdvancedExit] ${signal.symbol}: Bullish Divergence (Reversal)`);
                        }
                    }
                    ...
                    if (!shouldExit && exit_strategy.momentum?.enabled && ageHours >= exit_strategy.momentum.min_hours_check) {
                        ...
                        if (isLong && currentRSI > rsiThreshold && priceChange < maxFlat) {
                            ...
                            console.log(`ÔÜí [AdvancedExit] ${signal.symbol}: Momentum Dead (RSI ${currentRSI.toFixed(1)}, ${ageHours.toFixed(1)}h)`);
                        }

                        if (!isLong && currentRSI < (100 - rsiThreshold) && priceChange < maxFlat) {
                            shouldExit = true;
                            exitReason = 'MOMENTUM_EXHAUSTED';
                            console.log(`ÔÜí [AdvancedExit] ${signal.symbol}: SHORT Momentum Dead (RSI ${currentRSI.toFixed(1)})`);
                        }
                    }
                    ...
                    if (!shouldExit && exit_strategy.trailing_stop?.enabled && currentStage >= 1) {
                        ...
                        if (hitTrailing) {
                            ...
                            console.log(`­ƒôë [AdvancedExit] ${signal.symbol}: Trailing Stop (${trailingDistance.toFixed(2)} from max)`);
                        } else {
                    ...
                    if (!shouldExit && exit_strategy.time_decay?.enabled && ageHours >= exit_strategy.time_decay.soft_start_hours) {
                        ...
                        if (slChanged) {
                            ...
                            console.log(`ÔÅ▒´©Å [TimeDecay] ${signal.symbol}: SL tightened to ${tightenedSL.toFixed(2)} (${ageHours.toFixed(1)}h, factor: ${tightenFactor.toFixed(2)})`);
                            telegramService.sendUpdateAlert('SL_MOVED', {
                                symbol: signal.symbol,
                                oldSl: signal.stop_loss,
                                newSl: updates.stop_loss,
                    ...
                    if (!shouldExit && exit_strategy.time_decay?.forced_breakeven?.enabled) {
                        ...
                        if (ageHours >= forcedBEConfig.hours && currentStage <= forcedBEConfig.stage_threshold) {
                            ...
                            // Mover SL a breakeven si a├║n no est├í ah├¡
                            ...
                            if (isImprovement) {
                                ...
                                console.log(`ÔÅ░ [ForcedBreakeven] ${signal.symbol}: SL ÔåÆ Smart BE ($${updates.stop_loss}) after ${ageHours.toFixed(1)}h`);
                                telegramService.sendUpdateAlert('SL_MOVED', {
                                    symbol: signal.symbol,
                                    oldSl: signal.stop_loss,
                                    newSl: updates.stop_loss,
                    ...
                    if (!shouldExit && exit_strategy.time_decay?.enabled && ageHours >= exit_strategy.time_decay.hard_limit_hours) {
                        ...
                        if (Math.abs(pnlPercent) < 0.5) {
                            ...
                            console.log(`Ôîø [AdvancedExit] ${signal.symbol}: Closed by time (${ageHours.toFixed(1)}h, FLAT: ${pnlPercent.toFixed(2)}%)`);
                        }
                    }
                        ...
                        if (approachingEvent) {
                            ...
                            if (pnlPercent > eventProtection.actions.profitable_threshold) {
                                ...
                                if (isUnsecured) {
                                    ...
                                    console.log(`Ôÿó´©Å [NuclearGuard] ${signal.symbol}: Secured at BE (PnL ${pnlPercent.toFixed(2)}%) before ${approachingEvent.title}`);
                                    telegramService.sendUpdateAlert('SL_MOVED', {
                                        symbol: signal.symbol,
                                        oldSl: signal.stop_loss,
                                        newSl: updates.stop_loss,
                            ...
                            // B. WEAK TRADES -> EMERGENCY CLOSE (Avoid Slippage)
                                ...
                                if (isWeak) {
                                    ...
                                    console.log(`Ôÿó´©Å [NuclearGuard] ${signal.symbol}: Evacuating Weak Position (${pnlPercent.toFixed(2)}%) before ${approachingEvent.title}`);
                                }
                            }
                        }
                    }
    ...
    public async updateSignalContext(symbol: string, context: { new_tp?: number, reason?: string }) {
        ...
        if (signal && context.new_tp) {
            ...
            if (isSafer && Math.abs(context.new_tp - currentTargetPrice) > (currentTargetPrice * 0.002)) { // >0.2% change
                ...
                console.log(`­ƒôë [SignalAudit] Dynamic TP Adapted for ${symbol}: ${currentTargetPrice} -> ${context.new_tp}`);
     ...
     * Balance = Initial Balance + ╬ú(PnL de trades cerrados)
     */
    private async calculatePortfolioBalance(): Promise<number> {
        ...
        data.forEach((trade: any) => {
            const pnl = (trade.pnl_percent || 0) / 100; // Convert % to decimal (5% ÔåÆ 0.05)
            totalPnLDecimal += pnl;
        });

        // Balance = Initial ├ù (1 + Total PnL)
    ...
    public async getAdjustedRiskMultiplier(): Promise<{
        ...
        if (drawdownPercent > 0.1 || multiplier < 1.0) {
            const emoji = shouldStopTrading ? '­ƒøæ' : (multiplier < 1.0 ? 'ÔÜá´©Å' : '­ƒÆ░');
    ...
    public async forceAuditPass() {
        console.log("­ƒÜæ [SignalAudit] Manual Audit Pass Triggered...");
    ...
    public async getAdvancedMLMetrics() {
        ...
        try {
            ...
            console.log(`­ƒºá [ML-Audit] Advanced Stats Updated: ${winRate.toFixed(2)}% Accuracy | Drift: ${isDriftDetected ? 'ÔÜá´©Å YES' : 'Ô£à NO'}`);
            return this.mlBrainStatus;

        } catch (e) {
            console.error("ÔØî [ML-Audit] Failed to calculate advanced metrics:", e);
            return this.mlBrainStatus;
        }
    }
    ...
    public async getRecentSymbolHistory(symbol: string, hours: number = 4) {
        ...
        try {
            ...
            console.error(`ÔØî [SignalAudit] Error fetching history for ${symbol}:`, e);
            return [];
        }
    }
    ...
    private async checkHealthAndPoll(force: boolean = false) {
        ...
        if (force || (timeSinceLastTick > SILENCE_THRESHOLD && this.activeSignals.length > 0)) {
            if (!force) {
                console.warn(`ÔÜá´©Å [SignalAudit] WS Silence Detected (${(timeSinceLastTick / 1000).toFixed(0)}s). Engaging Proxy Polling...`);
            }
            ...
            try {
                ...
                if (Array.isArray(allPrices)) {
                    ...
                    for (const signal of this.activeSignals) {
                        try {
                            ...
                            console.error(`ÔØî [SignalAudit] Error processing ${signal.symbol}:`, signalErr.message);
                        }
                    }

                    this.lastPollSuccess = Date.now();
                    this.lastAuditError = null;
                    console.log(`­ƒÜæ [SignalAudit] Audit Pass Complete: Updated ${updatesCount} signals.`);
                ...
                console.error(`ÔØî [SignalAudit] Audit Pass Failed: ${e.message}`);
    ...
    private async runConsensusCheck() {
        ...
        try {
            ...
            if (data && data.price) {
                ...
                if (diffPercent > 1.0) {
                    const msg = `­ƒÜ¿ CRITICAL PRICE DEVIATION for ${symbol}: Stream=${streamPrice}, REST=${restPrice} (Diff: ${diffPercent.toFixed(2)}%)`;
                    ...
                    // console.log(`Ô£à [SignalAudit] Consensus OK for ${symbol} (Diff: ${diffPercent.toFixed(4)}%)`);
                }
            }
        } catch (e) {
